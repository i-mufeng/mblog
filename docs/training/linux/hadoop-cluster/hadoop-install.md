...
description: 本文将介绍 Hadoop 分布式数据库并在三台服务器上部署和搭建 Hadoop 集群，包含 HDFS、YARN、MapReduce 的基本配置。
categories:
  - Linux 分布式集群
outline: [ 2,3 ]
date: 2024-10-28
...
各节点规划如下：其中 Master 作为 NameNode、ResourceManager，同时也作为一个普通节点，运行 DataNode 以及 NodeManager。
...
启动完成后，分别在各个节点执行 JPS 命令即可查看进程内容，可以看到，我们三台服务器均按照预期启动进程。

![image-20241114113230328](https://cdn.imufeng.cn/mblog/202411151428590.png)

### 4.3 访问 WEB 端

Hadoop 提供了基础的 WEB-UI 查看集群信息，访问 Master 节点的 9870 端口即可：https://master.local:9870/
